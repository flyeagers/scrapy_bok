...
req_url_base = 'http://www.piaotian.com/html/0/'
...
def get_txt(txt_id):
    ...
    try:
        ...
        txt['title'] = soups.select('.title h1')[0].text
        txt['author'] = soups.select('.mainbody .list')[0].text
        ...
        first_page = soups.select('.mainbody .centent ul li')
        # 获取小说总章页面数
        section_ct = len(first_page)
        # 获取小说第一章页面地址
        the_page = first_page[0].select('a')
        first_page = the_page[0]['href']
        ...
        while 1:
            try:
                ...
                # 获取章节名称
                # section_name = soup.select('#main #content')
                # section_text = soup.select('#main #content table br')[1]
                ...
                section_text = soup_text.text
                #section_text = re.sub('\s+', '\r\n\t', section_text.text).strip('\r\n')
                # 获取下一章地址
                next_section = soup.find(attrs={'class': 'toplink'})
                txt_section = next_section.select('a')[2]['href']
                # txt_section = soup.select('#wrapper .content_read .box_con .bottem2 #A3')[0]['href']
                # 判断是否最后一章，当为最后一章时，会跳转至目录地址，最后一章则跳出循环
                if txt_section == 'index.html':
                ...
                # (section_text.text.encode('UTF-8'))
            except:
